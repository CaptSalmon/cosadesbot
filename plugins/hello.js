...
module.exports = {
  ...
  go: function(msg) {
    ...
    if (this.test(/\b(привет|прив\b)/, txt) && !this.test(/\bбот\b/, txt)) {
      ...
      return misc.randomChoice(['Хорошо!', 'Хорошо.', 'Плохо!', 'Плохо.', 'Как всегда.', 'А у тебя?', 'Чем занимаешься?', 'Я скальный наездник', 'Истинно познавшие чим не используют оценочных суждений.']);
    } else if (this.test(/\b(пока|бб)\b/, txt) && (msg.chat.type === 'private' || this.reply_to_me(msg) || this.test(/^(кай|косадес|наркоман|бот)\b/, txt))) {
      return misc.randomChoice(["Пока-пока, " + you + "!", "До встречи, " + you + "!", "Прочь, " + you + "!"]);
      ...
      return "Я не тупой!";
    } else if (this.test(/\b(нвах)\b/, txt)) {
      return "Ну да, я нвах.";
    ...
    } else if (this.test(/^\W*\b(кай|косадес|наркоман|бот)\b\W*$/, txt)) {
      return misc.randomChoice(['Что?', 'Что?', 'Что?', 'Да?', 'Да?', 'Да?', you, 'Слушаю', 'Я тут', 'Чё тебе, пёс?', 'С Л А В А   К Л И Н К А М']);
    } else if (msg.chat.type === 'private' || this.reply_to_me(msg) || this.test(/^(кай|косадес|наркоман|бот)\b/, txt)) {
      ...
      if (txt.endsWith('?')) {
        ...
        if (orMatch != null) {
          or1 = orMatch[1].trim();
          isCall = this.find(/^(кай|косадес|наркоман|бот)\b(.+)/, or1);
          if (isCall != null) {
            or1 = isCall[2];
          }
          or2 = orMatch[2];
